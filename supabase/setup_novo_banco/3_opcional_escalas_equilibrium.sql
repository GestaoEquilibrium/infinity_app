-- ═══════════════════════════════════════════════════════════════════════════
-- INFINITY — PASSO 3 (OPCIONAL): escalas reais da Equilibrium
--
-- Recria as 223 escalas dos 33 profissionais no banco novo.
-- PRÉ-REQUISITOS:
--   1. 1_schema_completo.sql e 2_empresa_e_admin.sql já rodados
--   2. Colaboradores cadastrados no módulo RH com os MESMOS nomes
--      (o script localiza cada profissional pelo nome — ex.: FRANCISCA%%)
-- Profissional não encontrado é simplesmente pulado, sem erro.
-- ═══════════════════════════════════════════════════════════════════════════
...
do $$
declare
  -- >>> Se sua empresa não se chamar 'Equilibrium', ajuste o nome abaixo:
  v_company_id uuid := (select id from companies where name = 'Equilibrium' limit 1);
  v_user_id uuid;
...
begin
  if v_company_id is null then
    raise exception 'Empresa não encontrada — ajuste o nome no topo do script.';
  end if;
...
-- ── Verificação ──
-- select count(*) from salas_escalas where company_id = (select id from companies where name = 'Equilibrium' limit 1);  -- esperado: até 223
-- select count(*) from salas_fechamentos f join salas_escalas e on e.id=f.escala_id where e.company_id=(select id from companies where name = 'Equilibrium' limit 1);  -- esperado: 4